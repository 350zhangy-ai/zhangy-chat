...
# -*- coding: utf-8 -*-
"""
AI 助手核心模块 - R3 MiniMind 集成版
基于 MiniMind 模型 + 知识库，支持本地部署
"""
...
class Assistant:
    """AI 助手类 (R3 MiniMind 集成版 - zhangy-chat)"""
    ...
    def __init__(self, config: Optional[Dict] = None,
        ...
        # zhangy-chat 模型（基于 MiniMind）
        self.model_wrapper = ZhangyChatModel(model_path)
        self.model = None
        self.tokenizer = None
        self._load_model()
    ...
    def _load_model(self):
        """加载 zhangy-chat 模型（基于 MiniMind）"""
        if self.model_wrapper.load():
        ...
        for category, data in self.knowledge_base.items():
            for keyword in data["keywords"]:
                if keyword in query:
                    ...
                    optimized_response = self._optimize_with_thinking(
    ...
    def _optimize_with_thinking(self, base_response: str, thinking_result: Dict) -> str:
        ...
        if personalization.get("structure") == "empathy_first":
            ...
            if emotion.get("needs_empathy"):
                empathy_prefix = self._get_empathy_prefix(
